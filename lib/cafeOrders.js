...
export default function cafeOrders(orderTimes, k) {
  const minutesArray = orderTimes.map((hours) =>  hours.split(":")[0] * 60 + +hours.split(":")[1]);
  let result = 0;
  let left = 0;
  let right = 0;

  while (right < orderTimes.length) {
    const interval = minutesArray[right] - minutesArray[left];
