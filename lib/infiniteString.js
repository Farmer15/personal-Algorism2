...
export default function infiniteString(s, t) {
  if (isSameStr(s, t)) return true;

  let [lognStr, shortStr] = [s, t];

  if (s.length < t.length) {
    [lognStr, shortStr] = [t, s];
  }

  if (lognStr.length % shortStr.length !== 0) return false;

  let count = lognStr.length / shortStr.length;
  let confirmStr = "";
  ...
  return confirmStr === lognStr;
}

function isSameStr(str1, str2) {
  const confrimChar = str1[0];
