...
export default function mostComplicatedOrder(orders) {
  let resultArray = [];
  let mostOrderCount = 0;

  orders.forEach((element) => {
    const orderArray = element.split(" ");
    const notDuplicatedOrderArray = new Set(orderArray);
    const orderCount = notDuplicatedOrderArray.size;

    if (orderCount > mostOrderCount) {
      mostOrderCount = orderCount;
      resultArray = [orderArray[0]];
    }

    if (orderCount === mostOrderCount && !resultArray.includes(orderArray[0])) {
      resultArray.push(orderArray[0]);
    }
  });
