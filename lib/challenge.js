...
export default function challenge(N, A, B) {
  ...
  for (const problemNum of A) {
    if (!objA[problemNum]) {
      objA[problemNum] = true;
    } else {
      return false;
    }
  ...
  for (const problemNum of B) {
    if (!objB[problemNum]) {
      objB[problemNum] = true;
    } else {
      return false;
    }
  ...
  for (const problemNum in objA) {
    if (objB[problemNum]) {
      count++;
    }
  }

  return [A.length - count, B.length - count, count];
}
