...
export default function calculateNumberBox(arr) {
  const sortedCopiedArr = [...arr].sort((a, b) => b - a);
  let result = 0;

  for (let i = 0; i <= sortedCopiedArr.length / 2 - 1; i++) {
    result += sortedCopiedArr[i] * 10 + sortedCopiedArr[sortedCopiedArr.length - 1 - i];
  }

  return sortedCopiedArr.length % 2 === 0 ? result : result + sortedCopiedArr[(sortedCopiedArr.length - 1) / 2];
}
