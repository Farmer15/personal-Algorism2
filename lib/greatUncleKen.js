...
export default function greatUncleKen(money) {
  ...
  for (const array of money) {
    resultArray = [...resultArray, ...spreadMoney(array)];
  }

  for (const money of resultArray.sort((a, b) => b - a)) {
    result >= 0 ? result -= money : result += money;
  }

  return result >= 0 ? result : -result;
}

function spreadMoney(array) {
  const resultArray = [];
  let moneyCount = array[1];

  while (moneyCount > 0) {
    resultArray[resultArray.length] = array[0];
    moneyCount--;
  }
